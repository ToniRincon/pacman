  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        ...
        successors.append(((nextPosition, tuple(nextCorners)), action, cost))
...
def cornersHeuristic(state, problem):
  ...
  # find distante to closest corner
  corner,distance = min([(corner,util.manhattanDistance(position,corner)) for corner in corners], key = lambda x:x[1])
  # new state form corner
  newCorners = list(corners)
  newCorners.remove(corner)
  newState = (corner,tuple(newCorners))
  # recurrence from corner
  return distance + cornersHeuristic(newState,problem)
...
def foodHeuristic(state, problem):
  ...
  food,distance = min([(food,util.manhattanDistance(position,food)) for food in foodGrid.asList()], key = lambda x:x[1])
  nextFood = foodGrid.copy()
  x,y = food
  nextFood[x][y] = False
  newState = (food,nextFood)
  return distance + foodHeuristic(newState,problem)
